    ...
    def test_barcodes(self):
        ...
        expected_dict = {
            "passengers_barcode.jpg": Barcode(
                code="4030521749429", type="EAN13", orientation="UP"
            ),
        ...
        for index, barcode_file in enumerate(barcode_files):
            ...
            self.assertEqual(
                len(barcode_list), 1, f"No barcodes found in {barcode_file}"
            )
        ...
        for barcode_file, barcode in barcodes.items():
            if barcode_file in expected_dict:
                ...
                self.assertEqual(
                    decoded_barcode.code,
                    expected.code,
                    f"EAN number does not match for {barcode_file}",
                )
                self.assertEqual(
                    decoded_barcode.type,
                    expected.type,
                    f"EAN type does not match for {barcode_file}",
                )
                self.assertEqual(
                    decoded_barcode.orientation,
                    expected.orientation,
                    f"Orientation does not match for {barcode_file}",
                )
