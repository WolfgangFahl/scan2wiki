...
from fb4.widgets import Copyright, Link, Menu, MenuItem, Widget
...
class Scan2WikiServer(AppWrap):
    ...
    def upload(self, path):
        ...
        html = self.render_template(
            template, title=title, activeItem=activeItem, uploadForm=uploadForm
        )
        return html
    ...
    def watchDir(self):
        ...
        if watchForm.validate_on_submit():
            ...
            pass
        html = self.render_template(
            template, title=title, activeItem=activeItem, watchForm=watchForm
        )
        return html
    ...
    def showArchives(self):
        ...
        """
        return self.showEntityManager(
            self.am, self.archiveRowHandler, self.archiveLodKeyHandler
        )
    ...
    def getFoldersAndFiles(self, name: str):
        ...
        if name in self.archivesByName:
            ...
            debug = True
            startMsg = pd.start(
                ArchiveManager.addFilesAndFoldersForArchive,
                kwargs={
                    "archive": archive,
                    "withOcr": True,
                    "store": True,
            ...
            return render_template(
                "progress.html", title=title, menu=self.getMenuList(), msg=startMsg
            )
        else:
    ...
    def showFolder(
        self, archiveName: str = None, folderPath: str = None, refresh: bool = False
    ):
        """
    ...
    def folderRowHandler(self, row):
        ...
        try:
            row["🔄"] = Link(
                self.basedUrl(
                    url_for(
                        "refreshFolder", archiveName=archiveName, folderPath=folderPath
                    )
                ),
                "🔄",
            )
            ...
            row["name"] = Link(
                self.basedUrl(
                    url_for(
                        "showFolder", archiveName=archiveName, folderPath=folderPath
                    )
                ),
                name,
            )
    ...
    def showFolders(self):
        ...
        """
        return self.showEntityManager(
            self.fm, self.folderRowHandler, self.folderLodKeyHandler
        )
...
class WatchForm(FlaskForm):
    ...
    home = str(Path.home())
    scandirField = FileField(
        "Scandir", [validators.DataRequired()], render_kw={"placeholder": f"{home}"}
    )
    submit = SubmitField()
...
class ProgressDisplayer:
    ...
    def start(self, functionToCall, kwargs):
        ...
        self.webserver.sseBluePrint.scheduler.add_job(
            functionToCall, "date", run_date=run_date, kwargs=kwargs
        )
